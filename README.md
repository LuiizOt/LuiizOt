# Hey there 👋

### About me
---
I'm Luiz Otávio — a Cartographic and Surveying Engineer who loves turning spatial data into meaningful insights.

🚀 Currently diving deep into a Data Science Bootcamp, where I'm learning to handle everything from data wrangling and visualization to predictive modeling and automation.

💼 I work in R&D, developing geospatial solutions and exploring ways to blend engineering, programming, and data science to solve real-world problems.

🧭 Passionate about GIS, Python, automation, and any project that mixes maps, data, and creativity.

Always learning. Always building. 🌍

### Contact
---
You can reach me via <a href="https://www.linkedin.com/in/luizot/">Linkedin</a> or e-mailing me!


### Sobre Mim
---
E aí! 👋
Sou o Luiz Otávio — Engenheiro Cartógrafo e Agrimensor (turma de 2023) apaixonado por transformar dados espaciais em insights significativos.

🚀 Atualmente estou me aprofundando em um Bootcamp de Ciência de Dados, aprendendo desde tratamento e visualização de dados até modelagem preditiva e automação.

💼 Trabalho com Pesquisa e Desenvolvimento (P&D), criando soluções geoespaciais e explorando maneiras de unir engenharia, programação e ciência de dados para resolver problemas do mundo real.

🧭 Sou apaixonado por GIS, Python, automação e qualquer projeto que envolva mapas, dados e criatividade.

Sempre aprendendo. Sempre construindo. 🌍

### Contato
---
Você pode entrar em contato via <a href="https://www.linkedin.com/in/luizot/">Linkedin</a> ou me mandando um e-mail!
